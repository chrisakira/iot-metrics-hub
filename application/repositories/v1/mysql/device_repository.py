"""
Mysql Device Repository Module for Application
Version: 1.0.0
"""
...
class DeviceRepository(AbstractRepository):
    ...
    def create(self, device: DeviceVO):
        ...
        keys.remove(self.PK)
        keys_str = ",".join(keys)
        values_count = len(device.values()) - 1
        ...
        del device_dict[self.PK]
        values = tuple(device_dict.values())

        # try to create
        try:
            created = self._execute(sql, values)
            # get last inserted id
            device.id = self.connection.insert_id()
            # commit
